...
(function(){
  ...
  function powerBadge(type){
    if(type==='dual_power')return '<span class="power-badge power-dual" title="Dual Power card" aria-label="Dual Power card"><span class="power-icon">⚡</span><span>DUO</span></span>';
    if(type==='autograph_defense')return '<span class="power-badge power-defense" title="Defense card" aria-label="Defense card"><span class="power-icon">🛡️</span><span>DEF</span></span>';
    return '';
  }
  ...
  matchupCard=function(m,i){
    ...
    return `<article class="panel matchup-card"><div class="matchup-top"><span>MATCHUP ${i+1}</span><span>${status}</span></div><div class="team-row ${aWin?'winner':''}"><div><div class="manager-line ${revealed&&m.card_type_a==='dual_power'?'powered-line':''}"><span class="manager-name">${esc(m.manager_a)}</span>${revealed?powerBadge(m.card_type_a):''}${revealed&&m.card_type_a==='dual_power'?'<span class="lightning-trail" aria-hidden="true">⚡</span>':''}</div><div class="captain-name">${esc(sideLabel(m,'a'))}</div></div><div class="score">${revealed?fmtScore(m.score_a):''}</div></div><div class="team-row ${bWin?'winner':''}"><div><div class="manager-line ${revealed&&m.card_type_b==='dual_power'?'powered-line':''}"><span class="manager-name">${esc(m.manager_b)}</span>${revealed?powerBadge(m.card_type_b):''}${revealed&&m.card_type_b==='dual_power'?'<span class="lightning-trail" aria-hidden="true">⚡</span>':''}</div><div class="captain-name">${esc(sideLabel(m,'b'))}</div></div><div class="score">${revealed?fmtScore(m.score_b):''}</div></div></article>`;
  };
